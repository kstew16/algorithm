...
fun main() = with(BufferedReader(InputStreamReader(System.`in`))){
    ...
    fun solve(i:Int,a:Int,b:Int,k:Int,added:Char){
        if(visited[i][a][b][k] || found) return
        ...
        if(i in 0 until n){
            ...
            if(a in 0 until n) solve(i+1,a+1,b,k,'A')
            // 문자열에 b 를 추가하면 현재 문자열의 a 값 만큼 k 가 증가됨
            if(b in 0 until n) solve(i+1,a,b+1,k+a,'B')
            // 문자열에 c 를 추가하면 현재 문자열의 a,b 의 수만큼 k가 증가됨
            if(i-a-b in 0 until n) solve(i+1,a,b,k+a+b,'C')
        }
        //ans[i-1] = ' '
    }
    solve(1,1,0,0,'A')
